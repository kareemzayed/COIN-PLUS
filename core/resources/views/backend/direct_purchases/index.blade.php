            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="card-header-form">
                        <form method="GET" action="{{ route('admin.direct.purchase.search') }}">
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" placeholder="اسم العنصر" name="item"
                                    value="{{ isset($search['item']) ? $search['item'] : '' }}">
                            ...
                            <tbody>
                                @forelse ($purchases as $key => $purchase)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($purchase->seller)
                                                <a href="{{ route('admin.user.details', $purchase->seller->id) }}"
                                                    style="text-decoration: none; font-size: 14px">
                                                    {{ optional($purchase->seller)->fullname }}
                                                </a>
                                            @else
                                                {{ $purchase->seller_on_way_name . ' (ONWAY) ' }}
                                            @endif
                                        </td>
                                        <td>
                                            <a href="{{ route('admin.funds.index', ['name' => $purchase->fund->name]) }}"
                                                style="text-decoration: none; font-size: 14px">
                                                {{ optional($purchase->fund)->name }}
                                            </a>
                                        </td>
                                        <td>
                                            {{ number_format($purchase->amount, 2) . ' ' . $general->site_currency }}
                                        </td>
                                        <td>
                                            {{ number_format($purchase->charge, 2) . ' ' . $general->site_currency }}
                                        </td>
                                        <td>{{ $purchase->note }}</td>
                                        <td>{{ $purchase->utr }}</td>
                                        <td>
                                        ...
                                        <td style="width:12%">
                                            @if (checkPermission(75))
                                                <button
                                                    data-href="{{ route('admin.direct.purchase.update', $purchase->id) }}"
                                                    data-purchase="{{ $purchase }}"
                                                    class="btn btn-md btn-info update"><i class="fa fa-pen"></i></button>
                                            @endif
                                            @if (checkPermission(76))
                                                <button
                                                    data-href="{{ route('admin.direct.purchase.delete', $purchase->id) }}"
                                                    data-purchase="{{ $purchase }}"
                                    ...
                                    @if (!empty($users))
                                        @foreach ($users as $user)
                                            <option value="{{ $user->account_number }}">
                                                {{ $user->fullname . ' - ' . $user->email . ' - ' . 'Balance: ' . number_format($user->balance, 2) . ' ' . $general->site_currency }}
                                            </option>
                                        @endforeach
                                    @endif
                                </datalist>
            ...
            $(function() {
                ...
                $('.add').on('click', function() {
                    ...
                    modal.find('form').attr('action',
                        '{{ route('admin.direct.purchase.create') }}');
                    modal.modal('show');
                })
